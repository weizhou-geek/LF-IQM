...
% other toolboxes.
% LF = preprocess_func('E:\VQA\metric\LF_IQM\Bikes');
% % load('E:\VQA\metric\LF_IQM\Bikes');
% for row = 1:size(LF,1)
%     for col = 1:size(LF,2)
%         CurrImg = squeeze(LF(row,col,:,:,:));
%         NormLF(row,col,:,:,:) = im2double(CurrImg);
%     end
% end
% LF = reshape(single(NormLF),101,1,720,960,3);
% LF = single(LF);
